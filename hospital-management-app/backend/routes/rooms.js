...
router.get('/', async (req, res) => {
  try {
    const [rows] = await db.execute('SELECT * FROM Room ORDER BY RoomID');
    res.json(rows);
  } catch (error) {
...
router.get('/:id', async (req, res) => {
  try {
    const [rows] = await db.execute('SELECT * FROM Room WHERE RoomID = ?', [req.params.id]);
    if (rows.length === 0) {
...
router.delete('/:id', async (req, res) => {
  try {
    const [result] = await db.execute('DELETE FROM Room WHERE RoomID = ?', [req.params.id]);
    if (result.affectedRows === 0) {
      return res.status(404).json({ error: 'Room not found' });
    }
